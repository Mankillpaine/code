    ...
    foreach($new_meta_boxes as $meta_box) {   
        ...
        switch ( $meta_box['type'] ){  
            ...
            case 'select':
                  ...
                  foreach ($bolgusers as $user){
//                      echo  '<option value="'.  $user->display_name . '">'. esc_html__($user->display_name).'( '.$user->user_nicename .' )' .'</option>';
                      if ( $meta_box['std'] == $user->ID ){

                          echo  '<option selected = "selected" value="'.  $user->ID . '">'. esc_html__($user->display_name).'( '.$user->user_nicename .' )' .'</option>';
                      } else{
                          echo  '<option value="'.  $user->ID . '">'. esc_html__($user->display_name).'( '.$user->user_nicename .' )' .'</option>';
                      }

                      var_dump($meta_box['std']);
                  }
                  echo '</select>';


                //echo '<select selected="1"><option>114111</option><option>114111</option> </select>';
                break;
			case 'uploader':   
                echo'<h4>'.$meta_box['title'].'</h4>';   
