...
if($platform){
    if($hourse>=9) {
        $sql = "SELECT a.*,users.`nickname`,users.`avatar`  from (select income_log.*  from `income_log` where `idate` = CURRENT_DATE()  and `platform` = ". $platform. " order by `increase_rmb`   desc limit 100) as a inner join users on  `a`.`platform` =`users`.`platform`  and `a`.`platformid` =users.`platformid` ORDER BY `a`.`increase_rmb` DESC";
    }else{
        $sql = "SELECT a.*,users.`nickname`,users.`avatar`  from (select income_log.*  from `income_log` where `idate` = CURRENT_DATE()-1  and `platform` = ". $platform . "  order by `increase_rmb`   desc limit 100) as a inner join users on  `a`.`platform` =`users`.`platform`  and `a`.`platformid` =users.`platformid` ORDER BY `a`.`increase_rmb` DESC";

    }
}else{
    if($hourse>=9) {
        $sql = "SELECT a.*,users.`nickname`,users.`avatar`  from (select income_log.*  from `income_log` where `idate` = CURRENT_DATE() order by `increase_rmb`  desc limit 100) as a inner join users on  `a`.`platform` =`users`.`platform`  and `a`.`platformid` =users.`platformid` ORDER BY `a`.`increase_rmb` DESC";
    }else{
        $sql = "SELECT a.*,users.`nickname`,users.`avatar`  from (select income_log.*  from `income_log` where `idate` = CURRENT_DATE()-1 order by `increase_rmb`  desc limit 100) as a inner join users on  `a`.`platform` =`users`.`platform`  and `a`.`platformid` =users.`platformid` ORDER BY `a`.`increase_rmb` DESC";
...
<div class="wapper" id="page">
    ...
    <div class="bigone" style="position:relative;overflow:hidden">
        <div style="position:relative;overflow:hidden">
            <div class="titleone">
                <a href="index.php"<?php if(!$_GET['platform']) {echo "style='color:#51a5fe;font-size:0.32rem;border-bottom:2px solid #51a5fe;'";} ?>>全网排行</a>
                <a href="index.php?platform=1" <?php if($_GET['platform']){echo "style='color:#51a5fe;font-size:0.32rem;border-bottom:2px solid #51a5fe; '";}   ?>  class="last">平台排行</a>
            </div>
            <div class="con">
                <div class="list1-title" <?php if($_GET['platform']){echo "style='display:block; '";}   ?>>
                    <a href="index.php?platform=1" <?php if($_GET['platform']==1){echo "style='color:#51a5fe;font-weight:bold; '";}   ?>>映客直播</a>
                    <!--                            <a href="javascript:;">熊猫TV</a>-->
                    <!--                            <a href="javascript:;">全民TV</a>-->
                    <a href="index.php?platform=3" <?php if($_GET['platform']==3){echo "style='color:#51a5fe;font-weight:bold; '";}   ?> >花椒直播</a>
                </div>
                <div class="date_line">
