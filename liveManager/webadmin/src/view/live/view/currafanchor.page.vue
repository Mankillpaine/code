 ...
 */
<template>
    <div id="p-currafanchor">
    	<header class="curr-header">
			<span class="title h-100 font-18 font-bold fl">今日增粉最高主播</span>
		...
		<section class="curr-main">
			<div class="table-thead clear">
				...
				<div class="table-th font-16 font-bold text-center fl">增粉</div>
				<div class="table-th font-16 font-bold text-center fl">总增粉</div>
			...
			<ul class="table-tbody">
				<li class="table-tr c-pointer clear" v-for="item of topFans">
...
const vm = {
    ...
    created() {
    	...
    	const 
    		vm = this,
    		API_TOFANS = vm.$store.state.api.todayall.topFans;

        vm.$http.post(API_TOFANS, {
    ...
    watch: {
        ['refTime'](curr) {
            ...
            // 异步调用新数据
            if (curr === 0) {
            	const API_TOFANS = vm.$store.state.api.todayall.topFans;
...
// 暴露组件配置
export default Object.assign({ name: 'p-currafanchor' }, vm);
</script>
