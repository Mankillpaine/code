 ...
 */
<template>
    <div id="p-curricanchor">
    	<header class="curr-header">
			<span class="title h-100 font-18 font-bold fl">今日收入最高主播</span>
		...
		<section class="curr-main">
			...
			<ul class="table-tbody">
				<li class="table-tr c-pointer clear" v-for="item of topIncome">
...
const vm = {
    ...
    watch: {
        ['refTime'](curr) {
            ...
            // 异步调用新数据
            if (curr === 0) {
            	const API_TOINCOME = vm.$store.state.api.todayall.topIncome;
...
// 暴露组件配置
export default Object.assign({ name: 'p-curricanchor' }, vm);
</script>
